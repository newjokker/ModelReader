...
- 支持自动增强：可用语言模型按句子判断语气，并插入停顿和情绪标记；未配置语气模型时自动使用本地规则
...
- `原文朗读`：不改写剪贴板文字
- `自动增强`：优先使用语气模型判断每句话的朗读语气，并插入 MiniMax 停顿和情绪标记；语气模型不可用时回退本地规则
...
语气模型只负责给句子标注 `plain`、`gentle`、`suspense`、`sad`、`news`、`energetic` 这几类语气，不改写原文。朗读记录里会保存语气来源、模型名和每句语气，不保存语气模型 API Key。
